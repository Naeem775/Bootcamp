...
exports.createBootcamp = catchAsync(async (req, res, next) => {
  ...
  if (publishedBootcamp && req.user.role !== 'admin') {
    return next(
      new APIError('This publisher has already published a bootcamp', 401)
    );
  }
...
exports.uploadBootcampPhoto = catchAsync(async (req, res, next) => {
  ...
  if (!bootcamp) {
    return next(
      new APIError(`No bootcamp found with this id ${req.params.id}`, 400)
    );
  }
  ...
  if (file.size > process.env.MAX_UPLOAD_SIZE) {
    return next(
      new APIError(`please upload file with size less than 1MB`, 400)
    );
  }
  ...
  file.mv(`${process.env.UPLOAD_PATH}/${file.name}`, async (err) => {
    if (err) {
      return next(
        new APIError('problem with file upload,please try again', 500)
      );
    }
